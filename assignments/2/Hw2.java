...
public class Hw2 {
    ...
    public static void main (String[] args) {
        org.junit.runner.JUnitCore.main("Hw2");
    }
    ...
    public void testRemoveDuplicates() throws Exception {
        int[] a = {1, 2};
        int[] b = {1, 2};
        Assert.assertEquals(b, StringUtils.removeDuplicates(a));

        int[] c = {1, 1};
        int[] d = {1};
        Assert.assertEquals(d, StringUtils.removeDuplicates(c));

        int[] e = {1, 2, 1};
        int[] f = {1, 2};
        Assert.assertEquals(f, StringUtils.removeDuplicates(e));

        int[] g = {};
        int[] h = {};
        Assert.assertEquals(h, StringUtils.removeDuplicates(g));
    }

    @Test
    public void testSlice() throws Exception {
        Assert.assertEquals("oc", StringUtils.slice("toucan", 1, 5, 2));
        Assert.assertEquals("acuo", StringUtils.slice("toucan", 4, 0, -2));
        Assert.assertEquals("uot", StringUtils.slice("toucan", 2, -1));
        Assert.assertEquals("ocn", StringUtils.slice("toucan", 1, 2));

        Assert.assertEquals("is", StringUtils.slice("This is what you have", 5, 7, 1));
        Assert.assertEquals("you", StringUtils.slice("This is what you have", -8, -5, 1));

        String s = "0123456789";
        Assert.assertEquals("01234", StringUtils.slice(s, 0, 5, 1));
        Assert.assertEquals("", StringUtils.slice(s, 0, 5, -1));
        Assert.assertEquals("", StringUtils.slice(s, 5, 1, 1));
